...
public class Weapon {
    ...
    public double spread, velocity = 20, recoil = 0;
    public boolean single = true;
    private static HashMap<String, String> config = new HashMap<>();

    public Weapon(String imagePath, int clipSize, int maxAmmo, double spread, long cooldown, double recoil, boolean burst) {
    ...
    public static Weapon loadXML(String filepath) {
        ...
        return new Weapon(config.get("sprite"), Integer.parseInt(config.get("clip")), Integer.parseInt(config.get("ammo")), Double.parseDouble(config.get("spread")), Long.parseLong(config.get("cooldown")), Double.valueOf(config.get("recoil")), Boolean.valueOf((config.get("burst"))));
    }
